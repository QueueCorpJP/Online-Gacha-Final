...
type PaymentIntentStatus = 'succeeded' | 'processing' | 'requires_payment_method' | 'default';
...
export default async function SuccessPage({ 
  ...
  const statusKey = (status as string in STATUS_CONTENT_MAP ? status : 'default') as PaymentIntentStatus;
      ...
      <div className="w-full max-w-md bg-white rounded-lg shadow-lg p-8 space-y-6">
        <div className="flex flex-col items-center space-y-4">
          <div 
            ...
            style={{backgroundColor: STATUS_CONTENT_MAP[statusKey].iconColor}}
          >
            {STATUS_CONTENT_MAP[statusKey].icon}
          </div>
          <h2 className="text-2xl font-semibold text-gray-900">
            {STATUS_CONTENT_MAP[statusKey].text}
          </h2>
        </div>
            ...
            <table className="w-full">
              <tbody className="divide-y divide-gray-200">
                <tr className="text-sm">
                  <td className="py-2 text-gray-500 font-medium">ID</td>
                  <td className="py-2 text-gray-900 font-mono">{paymentIntentId}</td>
                </tr>
                <tr className="text-sm">
                  <td className="py-2 text-gray-500 font-medium">Status</td>
                  <td className="py-2 text-gray-900">{status}</td>
                </tr>
              </tbody>
            </table>
          </div>
        ...
        <div className="flex flex-col space-y-3">
          {paymentIntent && (
            ...
            >
              ...
              <svg width="15" height="14" viewBox="0 0 15 14" fill="none" xmlns="http://www.w3.org/2000/svg" className="ml-2">
